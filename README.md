...
<a name="karate-fork"><code>karate.fork(map)</code></a> | executes an OS command, but forks a process in parallel and will not block the test like [`karate.exec()`](#karate-exec) e.g. `karate.fork({ args: ['/Applications/Google Chrome.app/Contents/MacOS/Google Chrome'] })` - you can use a single string as `line` (or `karate.fork(line)`) instead of `args`, and an optional `workingDir` string property is also supported - this returns a [`Command`](karate-core/src/main/java/com/intuit/karate/shell/Command.java) object which has operations such as `waitSync()` and `close()` if you need more control
