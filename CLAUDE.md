...
- **Tag non-code commits with `[no ci]`.** Docs-only and other commits that don't touch buildable/testable code should carry `[no ci]` in the subject line to skip the CI run. It must be added at commit time — `main` is protected against force-push, so it can't be applied retroactively once pushed.
