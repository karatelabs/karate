...
/**
 * A (possibly chained) token, where a token is a selector or a locator, which can be resolved into a {link com.microsoft.playwright.Locator} 
 *
 * Karate's friendly locators are supported and will be automatically converted into Playwright compatible locators (xpath, css, pseudo or even possibly custom locators).
 
 * In essence, a PlaywrightToken could just wrap a {link com.microsoft.playwright.Locator}. Since PW's Locators are by nature chained, they meet all the requirements of PlaywrightToken. 
 * Current implementation, however, is a bit more complicated. Locators are not stored internally but only computed when requested so the chaining is implemented within the class, hence the <pre>parent<pre> reference.
