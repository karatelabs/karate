...
> **Status (2026-05-28):** *In progress.* Placeholder splice machinery, icons sprite, Tailwind config skeleton, template port (Bootstrap → Tailwind), theme-aware navbar, `karate-report.js` rewrite with Tailwind classes, Bootstrap asset deletes, `etc/tailwind/tailwind.sh` build script (no node / npm / mojo / vendored binary, see §3.1), and the CI staleness gate in `cicd.yml` have all landed. **Still TODO:** replace `vis-timeline` CDN on the Timeline page (O20). Phase 1 closes when O20 is resolved.
...
- Replace `vis-timeline` CDN reference on the Timeline page (D1 "no CDN at view time") — see O20 for options (vendor, custom build, or documented limitation).
...
karate-core/
  src/main/resources/io/karatelabs/output/
    ...
    karate-timeline.html                   ✅  ditto (vis-timeline CDN still — O20 close P1)
