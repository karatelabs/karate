...
- Extend `StepResult.Embed` to the multi-part shape (§3.6) with the single-part legacy shim. ✅ done (`Embed` = `name` + `List<Part>` + `meta`; legacy single-asset embeds keep the flat wire shape, multi-part embeds emit `parts[]`+`meta`).
